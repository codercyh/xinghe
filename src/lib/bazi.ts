// 八字计算 - 客户端调用 API route 版本
// calculateBaziAsync 通过 /api/bazi 获取精确结果
// calculateBazi 是简化同步版（缘分合盘用，不需要农历精度）
...
// 通过 API route 获取精确八字（服务端计算，避开客户端打包问题）
...
export function calculateBazi(
  ...
  const dayGanIdx = CYCLE.indexOf(dayGanZhi) % 10;
  const hourGanIdx = ((wushudun[dayGanIdx] ?? 0) + hourZhiIdx) % 10;

  const gans = [TIAN_GAN[yearGanIdx], TIAN_GAN[monthGanIdx], dayGanZhi[0], TIAN_GAN[hourGanIdx]];
  const zhis = [DI_ZHI[yearZhiIdx], DI_ZHI[monthZhiIdx], dayGanZhi[1], DI_ZHI[hourZhiIdx]];

  const wuxing: WuxingCount = { metal: 0, wood: 0, water: 0, fire: 0, earth: 0 };
  gans.forEach(g => {
    const w = WUXING_MAP[g] as WuxingKey;
    if (w && w in wuxing) wuxing[w]++;
  });
  zhis.forEach(z => {
